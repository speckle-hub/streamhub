...
router.get('/home', async (req, res) => {
  try {
    ...
    let nsfwWatchlist: any[] = [];
    let nsfwProgress: any[] = [];
    try {
      ...
      nsfwWatchlist = results[0];
      nsfwProgress = results[1];
    } catch (dbErr: any) {
      console.warn('[NSFW Home] DB unavailable, skipping personal data:', dbErr.message);
    }

    const [adultTrending, hentaiTrending] = await Promise.all([
      addonProxy.fetchCatalog('porntube', 'movie', 'popular'),
      addonProxy.fetchCatalog('hentaistream', 'movie', 'popular')
    ]);
